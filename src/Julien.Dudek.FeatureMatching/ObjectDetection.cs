using OpenCvSharp;
namespace Julien.Dudek.FeatureMatching;

public class ObjectDetection
{
    public async Task<IList<ObjectDetectionResult>> DetectObjectInScenes(byte[] objectImageData,
        IList<byte[]> imagesSceneData)
    {
        IList<ObjectDetectionResult> objectDetectionResults = new List<ObjectDetectionResult>();
        foreach (var imageScene in imagesSceneData)
        {
            objectDetectionResults.Add(await Task.Run(() => DetectObjectInScene(objectImageData, imageScene)));
        }
